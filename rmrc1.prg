...
outlog(3, __FILE__, __LINE__, 'Дней модификации '+str(KolModr, 2)+' Период '+dtoc(dtr))
  ...
  if (netfile('dokk', 1))
    ...
    while (!eof())
      ...
      if (!netseek('t12', 'mnr,rndr,0,rnr'))
        ...
        prinsr=1
        while (mn=mnr.and.rnd=rndr.and.sk=0.and.rn=rnr)
...
function RmRCa1()
  ...
  outlog(3, __FILE__, __LINE__, 'Дней модификации '+str(KolModr, 2))

  ?'ASTRU'
  outlog(3, __FILE__, __LINE__, 'ASTRU')
  astrurc1()

  // Копирование файлов RC1=1,2

  ?'COMM'
  outlog(3, __FILE__, __LINE__, 'COMM')
  pathinr=PathmInr+gcDir_c
  pathoutr=PathmOutr+gcDir_c
  ...
  ?'ENT '
  outlog(3, __FILE__, __LINE__, 'ENT ')
  pathinr=PathmInr+gcDir_e
  pathoutr=PathmOutr+gcDir_e

  dirr=4
  RCfl(1)
  outlog(3, __FILE__, __LINE__, 'EntRC1()')
  EntRC1()
  ...
  for yy=yy1r to yy2r
    ...
    pathgoutr=pathoutr
    ?pathinr
    outlog(3, __FILE__, __LINE__, 'pathinr', pathinr)
    dirr=7
    rcfl(1)
    do case
    case (yy1r=yy2r)
    ...
    for mm=mm1r to mm2r
      ...
      if (dirchange(dirinr)=0)
        if (dirchange(diroutr)=0)
          ...
          dirr=2
          rcfl(1)
          outlog(3, __FILE__, __LINE__, 'rcfl(1)')
          BankRC1a()
        endif
...
function rcfl(p1)
  ...
  while (!eof())
    ...
    als_r=alltrim(als)
    pathr=pathinr
    iif(dirr=2,outlog(3,__FILE__,__LINE__,"pathr,als_r",pathr,als_r),NIL)
    if (!netfile(als_r, 1))
      skip
      loop
    endif
    ...
    while (!eof())
      ...
      skip
    enddo
    iif(dirr=2,outlog(3,__FILE__,__LINE__,"end eval als_r",als_r),NIL)
...
function BankRC1a()
  ...
  if (netfile('dokk', 1))
    netuse('dokk', 'dokkin',, 1)
    outlog(3, __FILE__, __LINE__, 'dokkin', pathr)
    pathr=pathoutr
    netuse('dokk', 'dokkout',, 1)
    outlog(3, __FILE__, __LINE__, 'dokkout', pathr)

    outlog(3, __FILE__, __LINE__, 'dokkin', dokkin->(LastRec()))
    sele dokkin
    go top
    while (!eof())
      mnr=mn
      ...
      if (!netseek('t1', 'mnr,rndr,kklr,rnr'))
        ...
        prinsr=1
        while (mn=mnr.and.rnd=rndr.and.kkl=kklr.and.rn=rnr)
          if (!prc.and.!(bs_d=99.or.bs_k=99))
            prinsr=0
            exit
          endif
    ...
    outlog(3, __FILE__, __LINE__, 'dokkout', dokkout->(LastRec()))
    sele dokkout
    set orde to tag t7
    if (netseek('t7', '0'))
      while (rmsk=0)
    ...
    if (netfile('nnds', 1))
      netind('nnds', 1)
      netuse('nnds', 'ndsin',, 1)
      outlog(3, __FILE__, __LINE__, 'ndsin', pathr)
      pathr=pathdoutr
      netuse('nnds', 'ndsout',, 1)
      outlog(3, __FILE__, __LINE__, 'ndsout', pathr)

      outlog(3, __FILE__, __LINE__, 'ndsout', ndsout->(LastRec()))
      sele ndsout
      go top
      while (!eof())
        mnr=mn
